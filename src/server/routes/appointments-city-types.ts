// api/appointments-city-types.ts
// Tipos de consulta (mesmo agrupamento do bloco Tipos) filtrados por cidade do mapa.
import {
  MDS_PARTNER_SCOPE,
  requireBasicAuth,
  requireMenuAccess,
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  const cidadeRaw = String(req.query.cidade || "").trim();
  if (!/^[A-Z]{2}$/.test(uf) || !cidadeRaw) {
    return res.status(400).json({ error: "Parâmetros uf e cidade são obrigatórios" });
  }
  ...
  try {
    ...
    const cidadeParam = params.add(cidadeRaw);
    ...
    const rows = await runQuery(
      ...
      FROM classified
      WHERE uf = ${ufParam}
        AND cidade_norm = ${normalizeCitySql(cidadeParam)}
    ...
    const withPct = items.map((item) => ({
      ...item,
      percentual: total > 0 ? Math.round(((Number(item.total) || 0) / total) * 1000) / 10 : 0,
    }));
    ...
    res.status(200).json({
      ...
      total,
      items: withPct,
      source: "KPI summarized + gold_live.cidade · mesmo agrupamento de Tipos de consulta",
      filters: {
