...
import { listAllPartners, listPartnerGroupMap } from "../../../../src/server/dashboard/partners";
...
export async function GET(request: NextRequest) {
  ...
  const [users, economicGroups, partners, partnerGroupMap] = await Promise.all([
    listManagedUsersPublic(),
    listAllEconomicGroups().catch(() => []),
    listAllPartners().catch(() => []),
    listPartnerGroupMap().catch(() => ({})),
  ]);
  return NextResponse.json(
    { users, menuCatalog: MENU_SECTIONS, economicGroups, partners, partnerGroupMap },
