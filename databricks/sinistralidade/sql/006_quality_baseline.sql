...
USING (
  ...
  FROM metrics LATERAL VIEW STACK(4,
    ...
    'gross_cost_reconciliation', CASE WHEN abs(silver_cost - gold_cost) <= 0.01 THEN 'passed' ELSE 'failed' END,
      cast(gold_cost AS DOUBLE), cast(silver_cost AS DOUBLE), cast(0.01 AS DOUBLE), 'Custo bruto reconciliado em centavos (tolerância aplicada de fato)',
